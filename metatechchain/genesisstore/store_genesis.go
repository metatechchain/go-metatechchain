...
func (s *Store) GetRules() metatechchain.Rules {
	cfg := s.rlp.Get(s.table.Rules, []byte("c"), &metatechchain.Rules{}).(*metatechchain.Rules)
	return *cfg
}

func (s *Store) SetRules(cfg metatechchain.Rules) {
	s.rlp.Set(s.table.Rules, []byte("c"), &cfg)
}

func (s *Store) GetGenesis() metatechchain.Genesis {
	meatadata := s.GetMetadata()
	return metatechchain.Genesis{
