...
func ApplyGenesis(statedb *state.StateDB, g metatechchain.Genesis, maxMemoryUsage int) (*EvmBlock, error) {
	mem := 0
...
func genesisBlock(g metatechchain.Genesis, root common.Hash) *EvmBlock {
	block := &EvmBlock{
		EvmHeader: EvmHeader{
			Number:   big.NewInt(0),
			Time:     g.Time,
...
func MustApplyGenesis(g metatechchain.Genesis, statedb *state.StateDB, maxMemoryUsage int) *EvmBlock {
