...
import (
	...
	"github.com/metatechchain/go-metatechchain/gossip/evmstore"
	"github.com/metatechchain/go-metatechchain/gossip/sfcapi"
...
func (s *Service) GetConsensusCallbacks() meta.ConsensusCallbacks {
	return meta.ConsensusCallbacks{
...
func consensusCallbackBeginBlockFn(
	...
	return func(cBlock *meta.Block) meta.BlockCallbacks {
		wg.Wait()
		start := time.Now()
...
func spillBlockEvents(store *Store, block *inter.Block, network metatechchain.Rules) (*inter.Block, inter.EventPayloads) {
...
func mergeCheaters(a, b meta.Cheaters) meta.Cheaters {
	if len(b) == 0 {
		return a
	}
	if len(a) == 0 {
	...
	aSet := a.Set()
	merged := make(meta.Cheaters, 0, len(b)+len(a))
	for _, v := range a {
