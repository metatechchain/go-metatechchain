...
func newTestEnv() *testEnv {
	genStore := makegenesis.FakeGenesisStore(genesisStakers, utils.ToMtc(genesisBalance), utils.ToMtc(genesisStake))
...
func (env *testEnv) callContract(
	...
	vmenv := vm.NewEVM(context, txContext, state, env.store.GetRules().EvmChainConfig(), metatechchain.DefaultVMConfig)
