...
import (
	...
	"github.com/metatechchain/go-metatechchain/inter/drivertype"
	...
	"github.com/metatechchain/go-metatechchain/metatechchain/genesis/driver/drivercall"
	"github.com/metatechchain/go-metatechchain/metatechchain/genesis/driver/driverpos"
	...
	netinitcall "github.com/metatechchain/go-metatechchain/metatechchain/genesis/netinit/netinitcalls"
...
func NewDriverTxGenesisTransactor(g metatechchain.Genesis) *DriverTxGenesisTransactor {
...
func (p *DriverTxPreTransactor) PopInternalTxs(block blockproc.BlockCtx, bs blockproc.BlockState, es blockproc.EpochState, sealing bool, statedb *state.StateDB) types.Transactions {
	...
	if sealing {
		...
		for oldValIdx := idx.Validator(0); oldValIdx < es.Validators.Len(); oldValIdx++ {
			...
			missed := metatechchain.BlocksMissed{
...
func (p *DriverTxListener) OnNewLog(l *types.Log) {
	...
	if l.Topics[0] == driverpos.Topics.UpdateNetworkRules && len(l.Data) >= 64 {
		...
		p.bs.DirtyRules, err = metatechchain.UpdateRules(p.bs.DirtyRules, diff)
		if err != nil {
