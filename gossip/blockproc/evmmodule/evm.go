...
func (p *EVMModule) Start(block blockproc.BlockCtx, statedb *state.StateDB, reader evmcore.DummyChain, onNewLog func(*types.Log), net metatechchain.Rules) blockproc.EVMProcessor {
	...
	return &MetatechchainEVMProcessor{
		block:         block,
		reader:        reader,
		statedb:       statedb,
		onNewLog:      onNewLog,
...
func (p *MetatechchainEVMProcessor) evmBlockWith(txs types.Transactions) *evmcore.EvmBlock {
	h := &evmcore.EvmHeader{
		Number:     p.blockIdx,
...
func (p *MetatechchainEVMProcessor) Execute(txs types.Transactions, internal bool) types.Receipts {
	...
	receipts, _, skipped, err := evmProcessor.Process(evmBlock, p.statedb, metatechchain.DefaultVMConfig, &p.gasUsed, internal, func(log *types.Log, _ *state.StateDB) {
		p.onNewLog(log)
	})
	if err != nil {
...
func (p *MetatechchainEVMProcessor) Finalize() (evmBlock *evmcore.EvmBlock, skippedTxs []uint32, receipts types.Receipts) {
