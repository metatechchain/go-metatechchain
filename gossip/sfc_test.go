...
//go:generate bash -c "cd ../../metatechchain-sfc && git checkout c1d33c81f74abf82c0e22807f16e609578e10ad8"
//go:generate bash -c "docker run --name go-metatechchain-sfc-compiler -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../metatechchain-sfc:/src -w /src node:10.5.0 bash -c 'export NPM_CONFIG_PREFIX=~; npm install --no-save; npm install --no-save truffle@5.1.4' && docker commit go-metatechchain-sfc-compiler go-metatechchain-sfc-compiler-image && docker rm go-metatechchain-sfc-compiler"
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../metatechchain-sfc:/src -w /src go-metatechchain-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; rm -f /src/build/contracts/*json; npm run build'"
...
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../metatechchain-sfc:/src -w /src go-metatechchain-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; sed -i s/runs:\\ 200,/runs:\\ 10000,/ /src/truffle-config.js; rm -f /src/build/contracts/*json; npm run build'"
//go:generate bash -c "cd ../../metatechchain-sfc && git checkout -- truffle-config.js; docker rmi go-metatechchain-sfc-compiler-image"
...
import (
	...
	"github.com/metatechchain/go-metatechchain/gossip/contract/driver100"
	"github.com/metatechchain/go-metatechchain/gossip/contract/driverauth100"
	"github.com/metatechchain/go-metatechchain/gossip/contract/netinit100"
	"github.com/metatechchain/go-metatechchain/gossip/contract/sfc100"
	"github.com/metatechchain/go-metatechchain/logger"
	"github.com/metatechchain/go-metatechchain/metatechchain/genesis/driver"
	"github.com/metatechchain/go-metatechchain/metatechchain/genesis/driverauth"
	"github.com/metatechchain/go-metatechchain/metatechchain/genesis/evmwriter"
	"github.com/metatechchain/go-metatechchain/metatechchain/genesis/netinit"
	"github.com/metatechchain/go-metatechchain/metatechchain/genesis/sfc"
	"github.com/metatechchain/go-metatechchain/utils"
)
...
func TestSFC(t *testing.T) {
	...
	_ = true &&
		...
		t.Run("SFC upgrade", func(t *testing.T) {
			...
			rr := env.ApplyBlock(nextEpoch,
				env.Contract(admin, utils.ToMtc(0), sfc100.ContractBin),
			)
	...
	t.Run("Direct driver", func(t *testing.T) {
		...
		rr := env.ApplyBlock(nextEpoch,
			env.Contract(admin, utils.ToMtc(0), anyContractBin),
		)
		require.Equal(1, rr.Len())
		require.Equal(types.ReceiptStatusSuccessful, rr[0].Status)
		newImpl := rr[0].ContractAddress
...
func cicleTransfers(t *testing.T, env *testEnv, count uint64) {
	...
	for i := uint64(0); i < count; i++ {
		...
		for i := range txs {
			from := (i)%accounts + 1
			to := (i+1)%accounts + 1
			txs[i] = env.Transfer(from, to, utils.ToMtc(100))
		}
