...
import (
	...
	"github.com/metatechchain/go-metatechchain/tracing"
)
...
// GetEventPayload returns Meta event by hash or short ID.
...
// GetEvent returns the Meta event header by hash or short ID.
...
func (b *EthAPIBackend) GetEVM(ctx context.Context, msg evmcore.Message, state *state.StateDB, header *evmcore.EvmHeader, vmConfig *vm.Config) (*vm.EVM, func() error, error) {
	...
	if vmConfig == nil {
		vmConfig = &metatechchain.DefaultVMConfig
	}
