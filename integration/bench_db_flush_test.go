...
func BenchmarkFlushDBs(b *testing.B) {
	...
	genStore := makegenesis.FakeGenesisStore(1, utils.ToMtc(1), utils.ToMtc(1))
	_, _, store, s2, s3, _ := MakeEngine(rawProducer, InputGenesis{
		...
		Metatechchain:      gossip.DefaultConfig(cachescale.Identity),
		MetatechchainStore: gossip.DefaultStoreConfig(cachescale.Identity),
		Meta:               abft.DefaultConfig(),
		MetaStore:          abft.DefaultStoreConfig(cachescale.Identity),
