...
import (
	...
	"github.com/metatechchain/go-metatechchain/utils/adapters/vecmt2dagidx"
...
func getStores(producer kvdb.FlushableDBProducer, cfg Configs) (*gossip.Store, *abft.Store, *genesisstore.Store) {
	gdb := gossip.NewStore(producer, cfg.MetatechchainStore)

	cMainDb := mustOpenDB(producer, "meta")
	cGetEpochDB := func(epoch idx.Epoch) kvdb.DropableStore {
		return mustOpenDB(producer, fmt.Sprintf("meta-%d", epoch))
	}
	cdb := abft.NewStore(cMainDb, cGetEpochDB, panics("Meta store"), cfg.MetaStore)
	genesisStore := genesisstore.NewStore(mustOpenDB(producer, "genesis"))
	return gdb, cdb, genesisStore
}

func rawApplyGenesis(gdb *gossip.Store, cdb *abft.Store, g metatechchain.Genesis, cfg Configs) error {
	_, _, _, err := rawMakeEngine(gdb, cdb, g, cfg, true)
	return err
}

func rawMakeEngine(gdb *gossip.Store, cdb *abft.Store, g metatechchain.Genesis, cfg Configs, applyGenesis bool) (*abft.Meta, *vecmt.Index, gossip.BlockProc, error) {
	...
	if applyGenesis {
		...
		if err != nil {
			return nil, nil, blockProc, fmt.Errorf("failed to write Meta genesis state: %v", err)
		}
	}

	// create consensus
	vecClock := vecmt.NewIndex(panics("Vector clock"), cfg.VectorClock)
	engine := abft.NewMeta(cdb, &GossipStoreAdapter{gdb}, vecmt2dagidx.Wrap(vecClock), panics("Meta"), cfg.Meta)
...
func makeEngine(rawProducer kvdb.IterableDBProducer, inputGenesis InputGenesis, emptyStart bool, cfg Configs) (*abft.Meta, *vecmt.Index, *gossip.Store, *abft.Store, *genesisstore.Store, gossip.BlockProc, error) {
...
func MakeEngine(rawProducer kvdb.IterableDBProducer, genesis InputGenesis, cfg Configs) (*abft.Meta, *vecmt.Index, *gossip.Store, *abft.Store, *genesisstore.Store, gossip.BlockProc) {
