...
import (
	...
	"github.com/metatechchain/go-metatechchain/cmd/cmdtest"
)

func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "metatechchain-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "metatechchain-test" in exec().
	reexec.Register("metatechchain-test", func() {
...
func exec(t *testing.T, args ...string) *testcli {
	...
	// Boot "metatechchain". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("metatechchain-test", args...)
