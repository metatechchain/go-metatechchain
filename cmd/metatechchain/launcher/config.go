...
import (
	...
	"github.com/metatechchain/meta-base/abft"
	"github.com/metatechchain/meta-base/hash"
	"github.com/metatechchain/meta-base/utils/cachescale"
	"github.com/naoina/toml"
	"gopkg.in/urfave/cli.v1"

	"github.com/metatechchain/go-metatechchain/evmcore"
	"github.com/metatechchain/go-metatechchain/gossip"
	"github.com/metatechchain/go-metatechchain/gossip/gasprice"
	"github.com/metatechchain/go-metatechchain/integration"
	"github.com/metatechchain/go-metatechchain/integration/makegenesis"
	"github.com/metatechchain/go-metatechchain/metatechchain"
	"github.com/metatechchain/go-metatechchain/metatechchain/genesisstore"
	futils "github.com/metatechchain/go-metatechchain/utils"
...
var (
	...
	// DataDirFlag defines directory to store Meta state and user's wallets
	...
	RPCGlobalGasCapFlag = cli.Uint64Flag{
		Name:  "rpc.gascap",
		Usage: "Sets a cap on gas that can be used in mtc_call/estimateGas (0=infinite)",
		Value: gossip.DefaultConfig(cachescale.Identity).RPCGasCap,
	}
	RPCGlobalTxFeeCapFlag = cli.Float64Flag{
		Name:  "rpc.txfeecap",
		Usage: "Sets a cap on transaction fee (in MTC) that can be sent via the RPC APIs (0 = no cap)",
		Value: gossip.DefaultConfig(cachescale.Identity).RPCTxFeeCap,
	}

	AllowedMetatechchainGenesisHashes = map[uint64]hash.Hash{
		metatechchain.MainNetworkID: hash.HexToHash("0x8895b98d25c653773a31be420a6a29d322a10107e76dff37dc694ad02ebacd01"),
		metatechchain.TestNetworkID: hash.HexToHash("0xc4a5fc96e575a16a9a0c7349d44dc4d0f602a54e0a8543360c2fee4c3937b49e"),
	}
)
...
type config struct {
	Node               node.Config
	Metatechchain      gossip.Config
	MetatechchainStore gossip.StoreConfig
	Meta               abft.Config
	MetaStore          abft.StoreConfig
	VectorClock        vecmt.IndexConfig
}

func (c *config) AppConfigs() integration.Configs {
	return integration.Configs{
		Metatechchain:      c.Metatechchain,
		MetatechchainStore: c.MetatechchainStore,
		Meta:               c.Meta,
		MetaStore:          c.MetaStore,
		VectorClock:        c.VectorClock,
		AllowedGenesis:     AllowedMetatechchainGenesisHashes,
	}
}
...
func getMetatechchainGenesis(ctx *cli.Context) integration.InputGenesis {
	...
	case ctx.GlobalIsSet(FakeNetFlag.Name):
		...
		fakeGenesisStore := makegenesis.FakeGenesisStore(num, futils.ToMtc(1000000000), futils.ToMtc(5000000))
...
func mayMakeAllConfigs(ctx *cli.Context) (*config, error) {
	...
	cfg := config{
		...
		Metatechchain:      gossip.DefaultConfig(cacheRatio),
		MetatechchainStore: gossip.DefaultStoreConfig(cacheRatio),
		Meta:               abft.DefaultConfig(),
		MetaStore:          abft.DefaultStoreConfig(cacheRatio),
		VectorClock:        vecmt.DefaultConfig(cacheRatio),
	}

	if ctx.GlobalIsSet(FakeNetFlag.Name) {
		_, num, _ := parseFakeGen(ctx.GlobalString(FakeNetFlag.Name))
		cfg.Metatechchain = gossip.FakeConfig(num, cacheRatio)
	}
	...
	var err error
	cfg.Metatechchain, err = gossipConfigWithFlags(ctx, cfg.Metatechchain)
	if err != nil {
		return nil, err
	}
	cfg.MetatechchainStore, err = gossipStoreConfigWithFlags(ctx, cfg.MetatechchainStore)
	if err != nil {
		return nil, err
	}
	cfg.Node = nodeConfigWithFlags(ctx, cfg.Node)
	if cfg.Metatechchain.Emitter.Validator.ID != 0 && len(cfg.Metatechchain.Emitter.PrevEmittedEventFile.Path) == 0 {
		cfg.Metatechchain.Emitter.PrevEmittedEventFile.Path = cfg.Node.ResolvePath(path.Join("emitter", fmt.Sprintf("last-%d", cfg.Metatechchain.Emitter.Validator.ID)))
	}

	if err := cfg.Metatechchain.Validate(); err != nil {
		return nil, err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "eth", "mtc", "dag", "sfc", "abft", "web3")
	cfg.WSModules = append(cfg.WSModules, "eth", "mtc", "dag", "sfc", "abft", "web3")
	cfg.IPCPath = "metatechchain.ipc"
	cfg.DataDir = DefaultDataDir()
	return cfg
}
