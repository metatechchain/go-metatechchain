...
import (
	...
	"github.com/metatechchain/go-metatechchain/cmd/metatechchain/launcher/metrics"
	"github.com/metatechchain/go-metatechchain/cmd/metatechchain/launcher/tracing"
	"github.com/metatechchain/go-metatechchain/debug"
	"github.com/metatechchain/go-metatechchain/flags"
	"github.com/metatechchain/go-metatechchain/gossip"
	"github.com/metatechchain/go-metatechchain/integration"
	"github.com/metatechchain/go-metatechchain/utils/errlock"
	"github.com/metatechchain/go-metatechchain/valkeystore"
	_ "github.com/metatechchain/go-metatechchain/version"
)

const (
	// clientIdentifier to advertise over the network.
	clientIdentifier = "go-metatechchain"
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-metatechchain command line interface")
...
func initFlags() {
	...
	nodeFlags = append(nodeFlags, metatechchainFlags...)
...
// metatechchain is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func metaMain(ctx *cli.Context) error {
	...
	genesisPath := getMetatechchainGenesis(ctx)
...
func makeNode(ctx *cli.Context, cfg *config, genesis integration.InputGenesis) (*node.Node, *gossip.Service, func()) {
	...
	valPubkey := cfg.Metatechchain.Emitter.Validator.PubKey
	if key := getFakeValidatorKey(ctx); key != nil && cfg.Metatechchain.Emitter.Validator.ID != 0 {
	...
	svc, err := gossip.NewService(stack, cfg.Metatechchain, gdb, signer, blockProc, engine, dagIndex)
	if err != nil {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	// Create a client to interact with local metatechchain node.
	...
	if ctx.GlobalBool(utils.ExitWhenSyncedFlag.Name) {
		go func() {
			for first := true; ; first = false {
				// Call mtc_syncing until it returns false
				time.Sleep(5 * time.Second)

				var syncing bool
				err := rpcClient.CallContext(context.TODO(), &syncing, "mtc_syncing")
				if err != nil {
					continue
				}
				if !syncing {
